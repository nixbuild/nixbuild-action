...
nix copy --derivation --to ssh-ng://nixbuild "${nix_args[@]}" "${nix_installables[@]}"

for x; do
  nix_installable="$(echo "$x" | \
    jq -r '. as $x | "\(env.FLAKE_URL)#\(env.FLAKE_ATTR).\($x.attr)"'
  )"
  name="$(echo "$x" | jq -r '.label')"
  title="Build $(echo "$x" | jq -r  '. as $x | "\(env.FLAKE_ATTR).\($x.attr)"')"
  shift
